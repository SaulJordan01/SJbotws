...
let isGametime = st.gameTime; // waktu yang dibutuhkan untuk game
let isPoindefect = st.poinDefect; // poin awal adalah poin yang harus di capai untuk menaikkan level
...
// -- thumbnail
let thumb = fs.readFileSync('./temp/fg.jpg'); // !! jangan ganti, mau ganti ada casenya

let fakethumb = fs.readFileSync('./temp/fake.jpg'); // !! jangan ganti, mau ganti ada casenya
...
`
  ...
  case 'author':
    ...
    await Fg.fakeLink(from, capt, thumb, 'Click in here', 'https://www.instagram.com/mrf.zvx', mek)
   /* Fg.sendContact(from, '6282223014661', 'owner', {
	 key: {
          fromMe: false,
...
*WHATSAPP*
- Kontak : ${Object.keys(Fg.contacts).length}
...
- Browser : *${Fg.browserDescription[1]}*
- Versi Browser : *${Fg.browserDescription[2]}*
...
- License : ${package.license}` 
  Fg.fakeLink(from, teks, thumb, tanggal, 'https://www.instagram.com/mrf.zvx', mek)
   break
 ...
 case 'typewriter':
  ...
  case 'hacker':
    ...
    Fg.sendButton(from, capt, msg.next(command), 'Next', prefix+command)
    break
  ...
  case "toimg":
   ...
   case 'pinterest':
    ...
    Fg.sendMessage(from, await getBuffer(pin), image, { quoted: mek, caption: 'Result from *PINTEREST*\n'+pin, thumbnail: fakethumb })
 break
 ...
 break
  ...
  case 'upsw':
    ...
    if (m.quoted && m.quoted.mtype === 'conversation' && !value) _m = Fg.sendMessage('status@broadcast', {
        text: m.quoted.text,
        textArgb: 0xffffffff,
        backgroundArgb: pickRandom(colors)
    }, 'extendedTextMessage')
    if (!m.quoted && value) _m =Fg.sendMessage('status@broadcast', {
        value,
        textArgb: 0xffffffff,
        backgroundArgb: pickRandom(colors)
    }, 'extendedTextMessage')
    if (m.quoted && value) _m = Fg.forwardMessage('status@broadcast', await m.quoted.cMod('status@broadcast', value))
 ...
 case 'getimg':
   try { 
     ...
     Fg.sendMessage(from, mage, image, { quoted: mek, caption: 'Result : database image', thumbnail: fakethumb })
     } catch {
       m.reply(msg.packoff)
     }
     break
 ...
 case 'getvid':
   try { 
     ...
     Fg.sendMessage(from, vid, video, { quoted: mek, caption: 'Result : database video' })
     } catch {
       m.reply(msg.packoff)
     }
     break
...
break
 ...
 case 'setthumb':
   ...
   q = m.quoted ? m.quoted : m 
   thumb = await q.download() 
   fs.writeFileSync(`./temp/fg.jpg`, thumb)
   m.reply(msg.done)
   } else {
     m.reply(msg.replyImg)
   }
...
*Link Dev* : ${i.link_dev}\n────────────────\n`;
 ...
 case 'tiktokaudio':
   ...
   Fg.sendMessage(from, buffer, video, {quoted: mek, caption: msg.done})
     ...
     buffer = await getBuffer(ttdl.result.nowatermark)
     if(!buffer) return m.reply('Error')
     Fg.sendMessage(from, buffer, document, {quoted: mek, mimetype: 'audio/mp4', filename: `Tiktokdownloader.mp3`})
   } else {
  ...
  case 'warn':
    ...
    warn = cekWarn(dia)
    if (warn === 3) {
      Fg.groupRemove(from, [dia]).catch((e) => {console.log(`*ERROR:* ${e}`)})
  ...
  case 'add':
    ...
    Fg.sendGroupV4Invite(from, user, invit[0].invite_code, invit[0].invite_code_exp, groupMetadata.subject , `Invite you, to join a group`)
    }
    } catch (e) {
      m.reply(msg.nonum)
    }
  ...
  case 'tebaklirik':
    ...
    if (from in Fg.game) {
        ...
        json = list[random]
        let caption = msg.soal(json.soal, (isGametime / 1000).toFixed(2), isPoingame).trim()
    Fg.game[from] = [
